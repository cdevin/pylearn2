...
class Recurrent(Layer):
    ...
    nonlinearity : theano function or MLP layer, optional
...
class RUGatedRecurrent(Recurrent):
    """
    A modification of a recurrent layer that uses the reset and update
    gates decribed in http://arxiv.org/abs/1406.1078
    ...
    init_bias : float, optional
        Set an initial bias to be added at each time step. Defaults to 0.
    reset_gate_init_bias : float, optional
        Set an initial bias to be added at each time step to the reset gate.
        Defaults to 0.
    update_gate_init_bias : float, optional
        Set an initial bias to be added at each time step to the update gate.
        Defaults to 0.
    svd : bool, optional
        Use singular value decomposition to factorize the hidden-to-hidden
        transition matrix with weights in U(-irange, irange) into matrices
    ...
    def set_input_space(self, space):
        ...
        b_z = sharedX(np.zeros((self.dim,)) + self.reset_gate_init_bias, 
                      name=self.layer_name + '_b_z')
        b_r = sharedX(np.zeros((self.dim,)) + self.update_gate_init_bias,
                      name=self.layer_name + '_b_r')
...
class MultiplicativeRUGatedRecurrent(Recurrent):
    """
    A version of a Recurrent layer that uses the gating 
    http://arxiv.org/abs/1406.1078 and uses a different recurrent
    (hidden) weight matrix for each input label.

    Because this layer needs to know the label indices, it includes
    a projection layer inside of it. This layer CANNOT be used
    after a projection layer.

    Parameters
    ----------
    proj_dim : int
        The dimensionality of the projection of each label
    max_labels : int
        The greatest value that will be input as an index
