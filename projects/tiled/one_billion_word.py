...
class OneBillionWord(dense_design_matrix.DenseDesignMatrix):

    valid_set_names = ["train","valid", "test"]
    def __init__(self, which_set, seq_len, brown = None, sentence = True):

        if not sentence:
            raise NotImplementedError("Shuffled wrods version is not implemented")
