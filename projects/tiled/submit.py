...
def sparse_linear(submit = False, make = False):
    ...
    if submit:
        ...
        state.save_path = preprocess("${PYLEARN2_EXP_RESULTS}/pentree_sparse_local1_rec/")
        PATH = state.save_path
    ...
    for i in xrange(num_exp):
        ...
        state.h3_col_norm = rng.uniform(1., 3.5)
        ...
        state.embed_dim = rng.randint(500, 1000)
        ...
        state.h3_kernel_shape = rng.randint(2, 3)
        state.h4_units = rng.randint(200, 1000)
        ...
        state.lr_sat =rng.randint(50, 200)
        state.decay = 10. ** rng.uniform(-2, -1)
...
def relu(submit = False, make = False):
    """
    2 sparse linear layer
    """
    state = DD()
    with open('exp/penntree_maxout_local_relu.yaml') as ymtmp:
        state.yaml_string = ymtmp.read()
    ...
    num_exp = 20
    if submit:
        TABLE_NAME = "pentree_sparse_local_linear_relu"
        db = api0.open_db("postgres://mirzamom:pishy83@opter.iro.umontreal.ca/mirzamom_db?table=" + TABLE_NAME)
        state.save_path = './'
    else:
        state.save_path = preprocess("${PYLEARN2_EXP_RESULTS}/pentree_sparse_local1_rec/")
...
def dense_brown(submit = False, make = False):
    "densly connected one layer"
    state = DD()
    with open('exp/penntree_dense_brown.yaml') as ymtmp:
        state.yaml_string = ymtmp.read()
    ...
    if submit:
        ...
        state.save_path = preprocess("${PYLEARN2_EXP_RESULTS}/pentree_dense_brown/")
        PATH = state.save_path
    ...
    for i in xrange(num_exp):
        ...
        state.seq_len = rng.randint(5,7)
        state.embed_dim = rng.randint(10,100)
        state.h1_num_pieces = rng.randint(2, 5)
...
if __name__ == "__main__":
    ...
    elif args.task == 'composite_relu':
        composite_relu(args.submit, args.make)
    elif args.task == 'relu':
        relu(args.submit, args.make)
    elif args.task == 'dense_brown':
        dense_brown(args.submit, args.make)
    else:
