...
from noisylearn.projects.tiled.iteration import FiniteDatasetIterator, FiniteDatasetIteratorCluster


class SequenceDesignMatrix(DenseDesignMatrix):

    def get_data(self):
        if hasattr(self, 'aux_targets'):
            self.X, np.zeros((1, 1), dtype = 'int32'), self.aux_targets
        else:
...
class SequenceClusterDesignMatrix(DenseDesignMatrix):

    def get_data(self):
        garbage = np.zeros((1, 1), dtype = 'int32')
        return self.X, garbage, garbage
    ...
    def iterator(self, mode=None, batch_size=None, num_batches=None,
        ...
        if rng is None and mode.stochastic:
            rng = self.rng
        return FiniteDatasetIteratorCluster(self,
                                     mode(self.num_examples, batch_size,
                                     num_batches, rng),
                                     data_specs=data_specs,
                                     return_tuple=return_tuple,
