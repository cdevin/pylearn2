...
class MaxoutConv(Layer):
    ...
    def set_input_space(self, space):
        ...
        setup_detector_layer(layer=self,
                input_space=space,
                rng=self.mlp.rng,
                irange=self.irange)
        ...
        dummy_detector = sharedX(self.detector_space.get_origin_batch(2)[0,:,:,:,:])

        dummy_p = max_pool_2d(input=dummy_detector, ds=self.pool_shape)
        dummy_p = dummy_p.eval()
        self.output_space = Conv3DSpace(shape=[dummy_p.shape[1], dummy_p.shape[2]],
                                        sequence_length = self.detector_space.sequence_length,
