...
def cifar10_conv():
    ...
    state.irange = 0.1
    state.random_filters = False
    state.description = 'dropout_sp'

    state.yaml_string = convolution_yaml_string

    ind = 0
    TABLE_NAME = "cifar10_conv_sp"
    db = api0.open_db("postgres://mirzamom:pishy83@opter.iro.umontreal.ca/mirzamom_db?table=" + TABLE_NAME)
    for lr in [0.05, 0.005, 0.0005]:
        state.init_learning_rate = lr
        sql.insert_job(experiment, flatten(state), db)
...
if __name__ == "__main__":
    ...
    parser = arg parse.ArgumentParser(description = 'job submitter')
    parser.add_argument('-t', '--task', choices = ['tfd_conv', 'mnist_conv', 'cifar10_conv'])
