...
from noisy_encoder.datasets.google_tfd import GoogleTFD, GoogleTFDAug
...
def make_data(which, seed = 2322, num_aug = 3, partition = False, partition_size = 50000):
    ...
    if which == 'train':
        ...
        data.X, data.y = corner_shuffle(data.X, data.y, (data.X.shape[0], 48, 48), num_aug, rng)
...
def make_data_aug(which, seed = 2322):
